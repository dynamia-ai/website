...
## 首次配置（由同事自己的 Agent 操作）

1. 确认所选 Agent 的执行环境可读写仓库、运行 Git、Node.js/npm、GitHub CLI 和 Lark CLI，并能检查网页预览。Node 使用网站维护者验证过的版本。
2. 已安装则复用。未安装：`npm install -g @larksuite/cli`。安装官方 Skills：先用 `npx skills add --help` 确认所选 Agent 的安装目标，再安装 `larksuite/cli`；至少需要 `lark-shared`、`lark-doc`。不要假设全局安装后所有 Agent 都能发现。若工具不支持 Skills 安装，直接读取官方仓库中对应 SKILL.md 及其引用文件，并核对 CLI `--help`。它们提供随 CLI 更新的参数和认证规则，必须阅读，不要复制另一人的登录配置。
...
5. 原样展示授权链接及 `lark-cli auth qrcode` 生成的二维码，让本人在浏览器完成授权。本人回复完成后，Agent 执行 `lark-cli auth login --device-code <本次 device_code>`，随后 `lark-cli auth status --json --verify`。登录成功不代表有权读取某篇文档，必须实际读取目标文档和素材验证。
